...
@export var mode := CameraView.PLAYERS
...
func _process(dt:float) -> void:
	...
	var too_far_apart := false
	var max_diff := Global.config.camera_max_index_difference_between_players
	if max_diff >= 0:
		too_far_apart = abs(players_data.get_first_place().river_tracker.get_index_on_river_float() - players_data.get_last_place().river_tracker.get_index_on_river_float()) >= max_diff
	
	if too_zoomed_out or too_far_apart:
...
func get_bounds() -> Rect2:
	if mode == CameraView.FULL_MAP: return map_data.gen.bounds
	if mode == CameraView.PLAYERS:
